...
def reply():
	...
	if media_msg == '1' and sender in sky_pics:
		...
		if count == 0:
			...
			cur.execute("INSERT INTO entries (sender, latitude, longitude, region, country) VALUES (?, ?, ?, ?, ?)", (sender, message_latitude, message_longitude, location[0], location[1]))
		else:
			print(f'updating current row for {sender}')
			cur.execute("UPDATE entries SET latitude = ?, longitude = ?, region = ?, country = ? WHERE sender = ?", (message_latitude, message_longitude, location[0], location[1], sender))
		db.commit()
